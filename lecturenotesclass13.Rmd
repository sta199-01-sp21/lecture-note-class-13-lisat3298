...
Point estimates are less acccurate than intervals because intervals encompass a larger range.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-lisat3298/manhattan.csv")
```
...
We can say with 95% confidence that that the mean price of 1 bedroom Manhattan apartment is within the range (2299, 2977) in dollars.
...
- Population: Mice not on the treatment
...
We can say with 95% confidence that that the true proportion of mice that died while not
on the treatment is within the range (0.690, 0.797).
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop, level = .90)
ci_mice_lived
```
...
We can say with 90% confidence that that the proportion of mice that lived on the treatment is within the range (0.552, 0.651).
